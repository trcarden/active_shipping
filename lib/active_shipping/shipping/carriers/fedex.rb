    ...
    class FedEx < Carrier
      ...
      def parse_rate_response(origin, destination, packages, response, options)
        ...
        root_node.elements.each('RateReplyDetails') do |rated_shipment|
          ...
          transit_time = rated_shipment.get_text('TransitTime').to_s if service_code == "GROUND_HOME_DELIVERY"
          #binding.pry if service_code == "GROUND_HOME_DELIVERY"
          ...
          if service_code == "GROUND_HOME_DELIVERY"
            delivery_range = home_ground_delivery_range(transit_time, options)
          else
      ...
      # Home ground delivery only provides service Tuesday - Saturday.
      def home_ground_delivery_range(transit_time, options)
        shipping_date = ship_date(options[:turn_around_time]) # Hours -> Days

        # If the shipping date is SUN then we need to advance to the next monday.
        shipping_date = shipping_date.advance(:days => 1) if shipping_date.wday == 0

        # Then we can add the transit time into the system.
        num_days_in_transit = parse_transit_times([transit_time]).first
        delivery_w_days = (2..6) #Tuesday - Saturday.

        date = next_valid_date_from(shipping_date, num_days_in_transit, delivery_w_days)
        [date, date]
      end
      ...
      def next_valid_date_from(start_date, advance_days, valid_w_days)
        valid_day = Proc.new { |date| valid_w_days.include?(date.wday) }
        ...
        while count < advance_days
          future_date += 1.day
          count += 1 if valid_day.call(future_date)
        end
      ...
      # Home business days are tuesday - saturday
      def home_business_day?(date)
